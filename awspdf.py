...
def index():
   ...
   pdf.cell(10)
   pdf.output('pdf_converson_file.pdf','F')
   print('test',type(pdf.output('pdf_converson_file.pdf','F')))
   new_list.append(pdf.output('pdf_converson_file.pdf','F'))
   print("wdq")
   
   data = open('pdf_converson_file.pdf','rb')
   AWS_ACCESS_KEY ='AKIAI52SJM5FHWRTXYBQ'
   AWS_ACCESS_SECRET_KEY = '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'
   bucket_name = 'infocuit'
   ...
   client.Bucket(bucket_name).put_object(Key='pdf_converson_keys_file.pdf',Body=data)
   print ('It worked!')
   print("https://s3.amazonaws.com/infocuit/pdf_converson_keys_file.pdf")
